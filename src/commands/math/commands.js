...
LatexCmds.mathbb = bind(Style, '\\mathbb', 'span', 'class="mq-ams mq-font"');
LatexCmds.mathcal = bind(Style, '\\mathcal', 'span', 'class="mq-cal mq-font"');
LatexCmds.mathfrak = bind(Style, '\\mathfrak', 'span', 'class="mq-frak mq-font"');
//text-decoration
// TASK: for now bind overbrace and underbrace to over and under line 
// implement this support if needed
LatexCmds.underline = LatexCmds.underbrace = bind(Style, '\\underline', 'span', 'class="mq-non-leaf mq-underline"');
LatexCmds.overline = LatexCmds.overbrace = LatexCmds.bar = bind(Style, '\\overline', 'span', 'class="mq-non-leaf mq-overline"');
...
LatexCmds.cancel = P(MathCommand, function(_, super_) {
  _.init = function() {
        super_.init.call(this, '\\cancel', '<span class="mq-non-leaf mq-cancel">&0<span class="mq-cancel-inner">' + '_'.repeat(50) + '</span></span>');
    };
});
var dotHtml = function(char, calls) {
  return '<span class="mq-non-leaf"><span class="mq-dot-recurring-inner">'
  + '<span class="mq-dot-recurring">' + char + '</span>'
  + '<span class="mq-empty-box">&0</span>'
  + '</span></span>';
};
LatexCmds.dot = P(MathCommand, function(_, super_) {
    _.init = function() {
        super_.init.call(this, '\\dot', dotHtml('\u02d9'));
    };
});
LatexCmds.ddot = P(MathCommand, function(_, super_) {
    _.init = function() {
        super_.init.call(this, '\\ddot', dotHtml('¨'));
    };
});
...
LatexCmds.left = P(MathCommand, function(_) {
  _.parser = function() {
    ...
    return optWhitespace.then(regex(/^(?:[([|\.]|\\\{|\\\||\\l(angle|[V|v]ert|brace|ceil|floor)(?![a-zA-Z]))/))
      .then(function(ctrlSeq) {        
        var open = ctrlSeq;
        if(ctrlSeq.charAt(0) === '\\') {
          open = open.charAt(1) === '|' ? 'Vert' : ctrlSeq.slice(ctrlSeq.charAt(1) === 'l' ? 2 : 1);
        }
        var char = _bracketMap[open];
        if(char) { open = char[0]; ctrlSeq = ctrlSeq + ' '; }
	// if (ctrlSeq=="\\langle") { open = '&lang;'; ctrlSeq = ctrlSeq + ' '; }
	// if (ctrlSeq=="\\lVert") { open = '&#8741;'; ctrlSeq = ctrlSeq + ' '; }
 //  if (ctrlSeq=="\\lvert") { open = '&#8739;'; ctrlSeq = ctrlSeq + ' '; }
 //  if (ctrlSeq=="\\lbrace") { open = '{'; ctrlSeq = ctrlSeq + ' '; }
 //  if (ctrlSeq=="\\lceil") { open = '&#8968'; ctrlSeq = ctrlSeq + ' '; }
 //  if (ctrlSeq=="\\lfloor") { open = '&#8970'; ctrlSeq = ctrlSeq + ' '; }
 //  if (ctrlSeq=="\.") { open = ' '; ctrlSeq = ctrlSeq + ' '; }
        return latexMathParser.then(function (block) {
          return string('\\right').skip(optWhitespace)
            .then(regex(/^(?:[\])|\.]|\\\}|\\\||\\r(angle|[V|v]ert|brace|ceil|floor)(?![a-zA-Z]))/)).map(function(end) {              
              var close = end;
              if(end.charAt(0) === '\\') {
                close = close.charAt(1) === '|' ? 'Vert' : end.slice(end.charAt(1) === 'r' ? 2 : 1);                
              }
              var char = _bracketMap[close];
              if(char) { close = char[1]; end = end + ' '; }
	      // if (end=="\\rangle") { close = '&rang;'; end = end + ' '; }
	      // if (end=="\\rVert") { close = '&#8741;'; end = end + ' '; }
       //  if (end=="\\rvert") { close = '&#8739;'; end = end + ' '; }
       //  if (end=="\\rbrace") { close = '}'; end = end + ' '; }
       //  if (end=="\\rceil") { close = '&#8969'; end = end + ' '; }
       //  if (end=="\\rfloor") { close = '&#8971'; end = end + ' '; }
       //  if (end=="\.") { close = ' '; end = end + ' '; }
